...
use crate::types::{
    Condition, FieldCondition, Filter, IsEmptyCondition, NestedFilter, OwnedPayloadRef, Payload,
    PointOffsetType,
};
        ...
        res = res
            || field_condition
                ...
                .map_or(false, |condition| {
                    eprintln!("values_count: {:#?}", condition);
                    eprintln!("p: {:#?}", p);
                    eprintln!("condition.check(p): {:#?}", condition.check(p));
                    condition.check(p)
                });
    }
    res
